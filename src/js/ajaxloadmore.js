...
window.addEventListener('load', () => {
  ...
  if (document.querySelector('.press-body__slider-wrapper') === null) {
    return;
  }
  else {
    if (window.location.href.indexOf('page') !== -1 && window.location.href.split('page/')[1].split('/')[0] === document.querySelector('.press-body__slider-wrapper').dataset.max) {
      document.querySelector('.press-body__more').parentNode.removeChild(document.querySelector('.press-body__more'));
    }

    else if (document.querySelector('.press-body__slider-wrapper').dataset.page === document.querySelector('.press-body__slider-wrapper').dataset.max) {
      document.querySelector('.press-body__more').parentNode.removeChild(document.querySelector('.press-body__more'));
    }
      ...
      const ajaxLoadMore = () => {
        ...
        if (loadButton !== undefined && loadButton !== null) {
          ...
          loadButton.addEventListener('click', () => {
            let currentPage = document.querySelector('.press-body__slider-wrapper').dataset.page;
            let max_pages = document.querySelector('.press-body__slider-wrapper').dataset.max;
            ...
            axios.post('/time_development/wp-admin/admin-ajax.php', params)
              .then(res => {
                let posts_list = document.querySelector('.press-body__slider-wrapper');
                ...
                if (document.querySelector('.press-body__slider-wrapper').dataset.page == document.querySelector('.press-body__slider-wrapper').dataset.max) {
